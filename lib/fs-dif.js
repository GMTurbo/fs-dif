...
var fsDif = function(options) {
  ...
  var setRemovalTimer = (function(duration) {
  ...
  var checkForRename = function(newFileData) {
    ...
    if (result) {
      newFileData.oldFileName = path.basename(cache.entries[index].fileName);
  ...
  var checkForMoved = function(newFileData) {
    ...
    //if we are going to rename, remove the old stale file
    if (result) {
      newFileData.oldDirectory = path.dirname(cache.entries[index].fileName);
      cache.entries.splice(index, 1);
    }
