...
var fsDif = function(options) {
  ...
  // callback map to prevent remove event from firing
  // with rename and moved events
  var prepCb = function(cb, eventData) {
    if (!cbMap[eventData.hash]) {
      cbMap[eventData.hash] = {
        events: [],
        cb: cb
      };
    }

    cbMap[eventData.hash].events.push(eventData.action);

    if( cbMap[eventData.hash].events.length == 2 ){
